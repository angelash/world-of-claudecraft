...
- Keep fork-local shell HTML/CSS changes in one contiguous block with stable ids/classes
  instead of scattering tiny conditionals across `index.html`, `hud.ts`, and `main.ts`.
- AI and NPC extensions belong in data registries and thin bridges: profile tables,
  localized line catalogs and matchers, provider adapters, and named integration helpers.
  Do not thread fork-only special cases deep through `server/game.ts` or `src/sim/sim.ts`
  when an existing seam can host them.
- When a monolith must call fork code, add one small named integration point and keep the
  upstream control flow around it intact.
- After upstream merges that touch generated i18n, wiki, or media artifacts, clear the
  conflict by taking one side and regenerating. Never hand-edit the generated outputs.
