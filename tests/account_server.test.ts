...
describe('account portal rate limiting (429)', () => {
  it('429s change-password past the per-IP cap, without writing', async () => {
    ...
    for (let i = 0; i < 20; i++) rateLimited(makeReq({}, ip));
    const beforeWrites = writes.filter((w) => w.sql.includes('UPDATE accounts SET password_hash')).length;
    const last = makeRes();
    ...
    expect(writes.filter((w) => w.sql.includes('UPDATE accounts SET password_hash')).length).toBe(beforeWrites);
  });
