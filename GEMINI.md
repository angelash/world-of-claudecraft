...
7. **Senior Engineering Standards**:
   ...
   - **Branch discipline for fork syncs**: keep `main` as the only local working branch for day-to-day development. Do not create feature, fix, chore, or agent-specific branches for ordinary work. The only allowed local-branch exception is a short-lived branch used while merging `upstream/main` (or another upstream sync branch) into the fork, then fast-forward `main` and delete the temporary branch. Keep `upstream/*` as fetched remote-tracking refs instead of long-lived local mirrors.
   - **Fork merge hygiene**: this repo is a long-lived fork, so fork-local behavior should prefer registries, handler maps, provider adapters, and small named hooks over deep edits to `server/game.ts`, `src/sim/sim.ts`, `src/ui/hud.ts`, `src/main.ts`, or scattered `index.html` conditionals. Keep shell markup changes contiguous, and resolve generated i18n/wiki/media conflicts by regenerating instead of hand-editing outputs.
