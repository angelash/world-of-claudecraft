...
7. **Senior Engineering Standards**:
   ...
   - **Branch discipline for fork syncs**: keep `main` as the only long-lived local working branch. Use a temporary local branch only for risky operations such as upstream merges, then fast-forward `main` and delete the temporary branch. Keep `upstream/*` as fetched remote-tracking refs instead of long-lived local mirrors.
