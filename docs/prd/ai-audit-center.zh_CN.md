...
- 能看到 AI 生命层运行指标：provider calls、success、error、历史 fallback 兼容计数、decision accepted/rejected、local reaction、generated events、memory writes、memory flush/prune 状态和延迟。
...
- 后台展示 AI 使用频率：1 分钟、5 分钟、1 小时、24 小时窗口的 job 数、成功、错误、历史 fallback 兼容计数、拒绝、本地反应。
...
| 运营 | AI 今天是否过于频繁？是否大量 provider error 或旧版 fallback 记录？ | 频率窗口、错误率、历史 fallback 兼容计数、token 估算。 |
...
- provider job 总数。
- provider success、provider error、历史 fallback 兼容计数。
...
- playerEntityId。
- sceneId、zoneId。
- provider source：codex、provider stub、local、legacy fallback。
...
- 当 Codex CLI 或模型 provider 返回精确 usage 时，保存 exact token 字段，并保留 estimated 字段用于没有 usage 的历史记录或本地反应。
- 增加按模型、realm、profile、scene、NPC 的成本聚合。
- 增加异常告警：高 provider error、旧版 fallback 记录异常增长、token 暴涨、rejected decision 激增、memory flush failure。
