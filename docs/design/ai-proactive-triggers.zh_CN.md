...
需要明确压制的“AI 味”句式分为几类，这些也应作为 prompt 和后处理规则的一部分：

- 转折或提纲式开头：例如“首先、其次、最后、不过、但是、总之、具体来说”，以及 `however`、`firstly`、`secondly`、`overall`、`to summarize`。句子应该直接落在内容上，不先搭讲解架子。
- 复述问题的答题腔：例如“你问的是……”“如果你要问……”“to answer your question”“you asked about”。角色不需要先复述玩家提问，再开始说话。
- 建议型脚手架：例如“我建议你……”“照我看……”“I would recommend…”。除非该角色本来就爱发号施令，否则不要先摆出顾问口吻。
- 总结归纳口吻：例如“重点是……”“归根结底……”“这说明……”“the point is”“this means”“at the end of the day”。这些像复盘结论，不像现场反应。
- 说话前的旁白：例如“某某开口前……”“他偷偷瞥了一眼……”“Before speaking…”。动作重的时刻可以直接输出动作片段，不要再包一层叙述。
- 工整议论文结构：例如“一方面……另一方面……”“不是……而是……”。除非角色人设本来就是学者、法官或布道者，否则尽量避免这种过分工整的句法。
- 空泛感官问句：例如“闻到了吗？”“你听见了吗？”“Smell that?”。如果要写感官，就落到具体对象，例如雨打木窗、灯油味、湿石头、骨灰、铜钱声。

推荐方向：

- 一口气，只说一个画面、一个判断或一个情绪。
- 能落地就落地，优先写眼前可见、可闻、可触、可联想到的东西。
- 允许半句、碎句、嘟囔、打断和轻微误会，但不要把句子修成客服回复或课堂答案。
...
- 11 个 `MobFamily` 已有明确的 routine planKind catalog。野兽会觅食、避火或守气味，类人怪会营地占位、退让或低声盘算，鱼人会浅滩觅食、溅水后撤或群体警报，蜘蛛会织网潜近、退回蛛网或静默守巢，狗头人会抢亮物、抱蜡烛躲避或护蜡烛，亡灵会向墓气漂近、回避活物暖意或聆听坟声。
- 巨魔、食人魔、元素、龙裔和恶魔也有独立 planKind：分食与营地嘟囔、堵路与领地压迫、元素共鸣、古老审视、恐惧挑拨等。所有 family scene event 都携带 `family:*` 和 `routine:*` 标签，便于 HUD、审计和后续动态文本复用。
- creature routine 的 `activeRoutineLastKind` 现在包含 family、planKind 和 reaction，例如 `creature:beast:keepDistanceFromFire:avoid`，后台能看出它是生态生活动作，而不是普通 avoid。
- 动作桥按 planKind 细化：靠近型普通野兽、类人、狗头人、鱼人、亡灵、巨魔、食人魔、龙裔和恶魔可触发开怪；营地低语、护蜡烛、鱼人警报、蛛网守巢、坟声聆听、巨魔嘟囔和食人魔领地压迫可触发呼叫同伴；回避型仍走服务器权威 flee，并继续受精英、稀有、首领、宠物、距离、等级和玩家战斗状态保护。
