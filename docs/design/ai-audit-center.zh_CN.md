...
| 能力 | 现状 | 主要文件 |
|---|---|---|
| 运行指标 | `AiLifeLayer.runtimeMetrics()` 记录 provider calls、success、error、历史 fallback 兼容计数、accepted/rejected、local reactions、generated events、memory writes、flush/prune/latency。 | `server/ai/life_layer.ts`、`server/game.ts` |
...
flowchart LR
  ...
  LifeLayer --> Provider["Codex CLI provider / 显式测试 provider stub"]
...
interface AiAuditRecord {
  ...
  sceneId: string;
  zoneId: string;
  providerSource: 'codex' | 'provider' | 'local' | 'fallback';
...
- `codex`：默认生产路径，`CodexCliProvider` 通过真实 `codex exec` 返回并通过 validator 的结果。
- `provider`：单测或集成测试显式注入的 `AiProvider` stub，用来验证编排、审计和 validator，不代表伪造 Codex CLI。
- `local`：不调用 provider 的本地语义反应，例如 item discard、scene inspection、quest rumor、boss memory。
- `fallback`：仅用于读取旧审计记录的历史来源。当前实现中 provider 报错不会生成本地 fallback decision。
...
1. AI usage and tokens
   - 每个窗口展示 provider jobs、accepted、rejected、errors、legacy fallback、local、tokens。
   - 明确显示 estimated。
...
- provider error 记录为 `provider_error`，`providerSource` 保留真实来源（通常是 `codex`），`decision` 为空，`outputTokens=0`，`reason/error` 写明 Codex CLI 的启动、超时、退出、schema 或输出异常。不会再生成本地 fallback decision。
