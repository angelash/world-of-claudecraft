...
- 普通奇点怪死亡后也会留下短期 `singularity` scene trace：`AiLifeLayer.handleSimEvents` 在收到普通怪 `death` 事件时按确定性个体 profile 判断是否为奇点，若是则写入 `worldTrace`、`worldDirectorState` 和 journal。玩家随后检视同一场景会看到 `sceneTraceSingularity`，HUD 使用该生物显示名而不是内部 id；这不生成额外掉落、不改 XP、任务、仇恨、leash 或角色持久状态。
...
- `tests/ai_world_traces.test.ts` 覆盖 trace 分类、lineId、TTL、同场景和同来源边界；`tests/ai_scene_inspection.test.ts` 覆盖 trace 优先于静态 anchor；`tests/server_ai_interact.test.ts` 覆盖真实 `discard` 到 `ai_inspect_scene` 的可玩链路，以及奇点怪死亡后再检视场景读到 singularity trace，并断言不新增物件、不改任务状态或主线快照。
