...
- 物件检视和场景检视现在还会读取持久化 `worldDirectorState` audit：服务端重启或 fresh `AiLifeLayer` 后，同区域的短期 mood 仍能以只读 memory signal 或区域 echo 的方式回到玩家眼前，同时继续按 player、scene、zone、TTL 隔离。
- 场景检视会把活跃 proposal 传给普通怪族群反应排序，让世界导演的区域 mood 不只出现在玩家/NPC 话题里，也能影响周边生物的即时肢体语言；该路径不执行 proposal，不改变 sim 状态，只生成带目标和 `director:*` 证据的个人 `aiSpeech`。
