...
- `tests/ai_codex_worker.test.ts` 覆盖 fake Codex 正常输出、损坏 JSON、结构缺字段、额外字段、非法 intent 名称、stderr 截断，以及从 `AiLifeLayer.handleNpcInteraction` 真实走 `CodexCliProvider` fake CLI 的链路，确保接入不只是“能调用”，还要能拒绝坏输出、产出可见 `aiSpeech` 并维持 server overlay 的安全边界。
- `AiLifeLayer` 捕获 provider error 后会记录 `provider_error`，再生成本地 profile fallback decision 并重新走 `validateAiDecision`；真实 server 命令路径已覆盖 Codex worker 超时时玩家仍收到安全 `lineId` 反馈，且 quest log、已完成任务、战斗、掉落和经济不被改动。
...
- `AiLifeLayer.handleObjectInspection` 已把 `object_inspected` 接入 scene frame、decision journal、短期 item rumor 和附近对象兴趣反应。
...
- `tests/ai_object_reactions.test.ts` 覆盖 forge、lake、singularity 的语义分流；`tests/server_ai_interact.test.ts` 覆盖检视墓地物件不拾取、不改任务，检视副本门不进本，以及检视物件后 NPC 产生即时反应并在 gossip 中提起。
