...
- `server/ai/content_coverage.ts` 已提供内容覆盖报告，覆盖 MobFamily 语义深度、关键 NPC profile 的场景/物件/时间天气/记忆钩子、场景 anchor 和语义物件深度、所有可丢弃物品语义信号、AI lineId 注册情况；`tests/ai_content_coverage.test.ts` 会在新增内容缺语义时直接失败。
...
- 把 coverage 报告接入后续内容审查节奏，并在扩展新 zone、新族群、新物件 inspect 时同步提升人工检查清单。
