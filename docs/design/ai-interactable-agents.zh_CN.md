...
- `tests/ai_memory_persistence.test.ts` 覆盖本阶段可玩链路：丢弃物件后的本地反馈不被 DB 写入拖慢，持久化信号能进入 NPC 和物件 Codex job context，重启式 fresh `AiLifeLayer` 能在场景检视中恢复区域 director echo，持久化故障时玩家仍收到反馈且 pending 写入保留在诊断里，并覆盖过期清理成功、防重入和清理失败诊断；`tests/server_ai_interact.test.ts` 覆盖真实 `GameServer` 定时触发 `PgAiMemoryDb.pruneExpired()`，确认使用当前 sim time。
