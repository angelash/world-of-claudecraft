...
- 已补事务 fake 型重启长链路测试：同一持久化 fake 上 fresh `AiLifeLayer` 能读到重启式区域余波，执行 admin clear 后新的 fresh layer 不再读到旧 director echo，并继续断言 quest log 和已完成任务不变。后续仍可增加真实 Postgres 容器长跑，覆盖多玩家、多区域、多次保存和关闭开关回退的长期运行场景。
