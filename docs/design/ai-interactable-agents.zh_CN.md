...
- `tests/ai_memory_persistence.test.ts` 覆盖本阶段可玩链路：丢弃物件后的本地反馈不被 DB 写入拖慢，持久化信号能进入 NPC Codex job context，持久化故障时玩家仍收到反馈且 pending 写入保留在诊断里。
...
- 增加真实 Postgres 或事务 fake 的长链路保存回读、重启恢复、批量过期清理和关闭开关回退测试，覆盖多玩家、多区域和跨 session 的长期运行场景。
...
- `AiLifeLayer.runtimeMetrics()` 已提供第一版运行指标快照，统计 provider calls、成功、错误、fallback、accepted/rejected decision、local reaction、生成事件数、记忆写入队列、持久化失败、provider 延迟和最近错误；`tests/ai_decision_journal.test.ts`、`tests/server_ai_interact.test.ts`、`tests/ai_memory_persistence.test.ts` 覆盖成功、拒绝、provider fallback、本地反应和 DB 故障指标。
...
- admin Usage 页已显示 AI 生命层健康面板，包含 provider fallback、错误、accepted/rejected decision、local reaction、生成事件、记忆写入、持久化失败和延迟；`tests/admin_ai_metrics_ui.test.ts` 覆盖健康态、异常态和错误文本转义。
