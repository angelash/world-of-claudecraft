...
- `tests/ai_world_traces.test.ts` 覆盖 trace 分类、lineId、TTL、同场景和同来源边界；`tests/ai_scene_inspection.test.ts` 覆盖 trace 优先于静态 anchor；`tests/server_ai_interact.test.ts` 覆盖真实 `discard` 到 `ai_inspect_scene` 的可玩链路，并断言不新增物件、不改任务状态。
