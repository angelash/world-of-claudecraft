...
- `server/ai_memory_db.ts` 已提供持久化能力：幂等 `ai_memory_records` DDL、按 realm/player/scene/zone/kind/scope 查询的索引、结构化 audit record upsert、读取归一化和过期清理；`ensureSchema()` 已接入该 DDL，测试覆盖 schema、保存读取 round-trip、异常 payload 丢弃、同场景加同区域回读、跨玩家隔离、过期过滤和 bounded prune。
...
- `/admin/api/ai/memory/clear` 和 Usage 面板的 `Clear AI memory` 按钮可清理当前进程内的 volatile AI overlay，包括 NPC memory、rumor、world trace、creature memory、creature plan、boss memory、boss phase cue、world director state、decision journal 和 pending memory writes；它不删除角色存档、不改任务、背包、XP、等级、经济、战斗和宠物状态，也不直接改 Postgres 已持久化的历史 audit。
