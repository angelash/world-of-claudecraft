...
- `tests/ai_social_memory.test.ts` 覆盖互动记忆、rumor 衰减、过期、同场景、同区域、跨区域和同来源边界；`tests/server_ai_interact.test.ts` 覆盖真实 `discard` 到 NPC gossip 的同场景和同区域跨 NPC 可玩链路，并断言任务状态不变。
...
- 补齐任务完成后的传闻传播验证，并在后续世界导演阶段扩展跨区域、跨阵营的受控传播。
