...
主动触发、事件队列和 5 分钟定时轮询机制已拆出专题归档：`docs/design/ai-proactive-triggers.zh_CN.md`。本文继续作为 AI 生命层主方案，主动机制细节以后以该专题文档为准。
...
最近一轮真实审计记录和本地回归样本说明，当前系统已经能做到“结构合法、场景相关、不过度 AI 腔”，但还没有稳定做到“像活人在跟玩家说话”。问题不是单点，而是多层一起把输出磨平了。

### 已观察到的典型问题

- 话题标签回声：prompt 里直接出现 `Topic: recent`、`Topic: rumor` 这类标签，模型容易把它们回显成 “Recent?”、“最近？” 作为起手。
- 场景卡片化：回复会列出几个感官词或场景词，但没有态度、社交动作和说话对象感，像 hover 文本，不像对话。
- 情绪泄漏不足：上下文虽然有 `dayEnergy`、`nightFatigue`、`fogFear`、`undeadPressure` 等数值，但 prompt 没有要求把这些转成口气和情绪。
- 正向支架太弱：现有规则大量强调“不许怎样”，但对“怎样才像人说话”给得不够具体，模型容易退到最安全的说明式句子。
- 指纹示例误导：少数 `favoriteStarts` 本身就偏空泛，例如 “Smell that” 或 “Do you hear that”，会把模型引向低信息感官问句。
- 后处理损伤语义：当前 polish 和 shorten 规则有时会把前置小问句保留下来，却把后面真正有信息和情绪的句子裁掉。
- 节奏扁平：英文里把分号统一改成逗号，会把本来像口头停顿的两拍，压成拧巴的书面逗号句。

### 这类问题在行业方案里的共识

- Inworld 和 Convai 都把角色设计拆成角色背景、目标、情绪、关系、知识和动作，而不是只给一段风格描述。也就是说，“像谁说话”必须和“现在想干什么、在意什么、对谁说”绑在一起。
- NVIDIA ACE 强调角色体验是语音、表情、动画、意图和上下文的整体，不是单独一段文本。对本项目来说，即使先不做全语音，也要让文本带上可感知的情绪和反应意图。
- Ubisoft 的 NEO、Ghostwriter 等实践强调作者先定义角色骨架和边界，AI 再在骨架里即兴。意味着我们不能只给抽象 persona，还要给“回答方式”和“坏例子禁止项”。
- Generative Agents 说明可信角色来自观察、记忆、反思和计划的连续链条。只给场景事实，不给角色此刻的压力和立场，输出就会自然退回中性摘要。

### 本轮设计改造

#### 1. 把玩家问题从“标签”改成“真实意图”

- prompt 不再只写 `Topic: recent`，而是改成类似“the player wants to know what has been happening here lately”。
- 这样模型看到的是玩家意图，不是内部枚举名，能显著减少标签回声。

#### 2. 补正向的“对话感”约束

- 对 `npc_question` 明确要求：这是对玩家的 live reply，不是 caption、tooltip、report。
- 增加“两拍式”回答指导：先答，再加一个小的人味边缘，例如怀疑、疲惫、护短、干笑、敬畏、烦躁或压低声音。
- 明确要求“不要把场景词堆成列表”，而要把细节绑到角色关心的事上。

#### 3. 把场景 mood 转成口气压力

- 从 `scene.mood` 和 `scene.danger` 提取当前最强的 1 到 3 个情绪压力，例如疲惫、潮湿烦躁、雾中警惕、亡灵压迫、晴夜敬畏。
- 在 prompt 中显式提示这些压力应如何渗进语气，而不是只保留原始数值。

#### 4. 修正 speech fingerprint 的空泛起手

- 清理会诱导出低信息问句的 `favoriteStarts`。
- 示例：把 “Smell that” 替换成更落地的 “Bitter sap on the air”，把 “Do you hear that” 替换成更具体的 “Wind shifted west”。

#### 5. 后处理从“只收短”改成“保留有用的两拍”

- `dynamicText` 从“强行一短句”改成“一个 spoken turn，通常是一句，必要时两拍”。
- polish 层新增 topic echo 去除，例如 `Recent? ...`、`最近？...`。
- shorten 逻辑要避免只保留一个空前缀问句，必要时保留后面的真正回答。
- 英文分号不再一律压成逗号，而是优先还原成更接近口语停顿的句间节拍。

#### 6. 审计继续保留 generated / delivered 差异

- 现有审计链已经能同时看到 `generated_text` 和 `delivered_text`。
- 这轮之后，这对差异不只是后台诊断数据，也是对话质量回归的一部分：任何只剩标签、只剩空问句、只剩场景题眼的 delivered 文本，都视为缺陷。

### 预期结果

- NPC 回答玩家时，更像“带情绪地答你一句”，而不是“报一条现场信息”。
- 同一个场景事实，在不同 NPC 嘴里会更明显地带出立场差异。
- 通过 validator 的文本不再因为后处理而丢掉真正的人话部分。
- 主动 AI、社交序列和普通问答会共享同一套“去助手味 + 增加对话感”的规则，不再一边修好了，一边还在说标签和说明句。
...
- `server/ai` 只在在线服务器中启用。离线客户端和 headless 没有 Codex CLI provider 时必须提示明确异常，不再生成本地假 AI 台词。
- AI 输出不直接进入数据库或 `Sim`，必须先经过 schema、policy、cooldown、range、locale 和权限校验。
- 测试可以显式注入 `AiProvider` stub 来验证服务端编排和 validator，但不能伪造 Codex CLI 进程或把 stub 包装成“已跑通 Codex”。
...
| `dynamicText` speech | 中到高 | 仅实验 realm 显式开启，默认和正式路径关闭，稳定内容回收为 lineId。 |
...
- 任意失败都收束为 validator rejection 或 provider error，不抛到主循环，也不生成伪造 AI 反馈。玩家能看到明确异常，审计记录保留可读原因。

安全 lineId 生命层默认可用，保证已经展示的询问、检视和场景反馈入口不会空点。`AI_LIVING_WORLD_EXPERIMENT=0` 用于显式关闭整层 AI 表现，回退到原始任务、战斗、拾取和副本流程。更激进的 dynamicText、真实逃跑、微任务和自动导演执行仍必须使用单独实验开关或实验 realm。
...
6. 如果失败，记录 `canon_guard_rejected` 并只允许安全表现 intent，例如 `lookAt`、`pause`、`commentOnScene`；无法安全表达时向玩家提示 AI 响应被拒绝，不用本地假决策冒充模型。
...
当前项目要求所有玩家可见文本通过 `t()`、entity i18n、sim/server matcher 或明确的 lineId 事件本地化。AI 生命层也遵守同一规则：默认路径、正式路径和所有提交进主分支的玩家可见文案都必须有本地化 key，并且本方案新增的 AI 文案必须同步补齐 `zh_CN` 简中覆盖，不能让中文环境回退英文。

AI 生命层新增或改动文案时按以下规则执行：

- HUD 和客户端 chrome 文案必须进入 `src/ui/i18n.catalog/hud_chrome.ts` 或既有对应 catalog，并通过 `t()` 渲染。
- 服务端和 `src/sim` 不直接调用 `t()`；它们发给玩家看的内容必须是稳定 lineId 和插值值，或在客户端边界通过 matcher 重新本地化。
- `hudChrome.aiQuestion.*`、`hudChrome.aiReaction.*`、`hudChrome.aiSpeech.*` 和 AI 相关入口 key 新增时，必须同步补 `src/ui/i18n.locales/zh_CN.ts`，简中不能处于 pending。
- Codex CLI 或 provider 输出在默认路径只能选择允许表中的 lineId 和 values。`dynamicText` 只能在显式实验开关中使用，且必须标记玩家 locale、通过过滤、只进入实验事件，不得混入正式或默认路径。
- 实验中表现优秀的 dynamicText 在进入默认体验前必须沉淀为 lineId，由作者审核并补齐简中翻译。
- 提交前必须运行 `npm run i18n:build`、`npm run i18n:scan` 和相关本地化测试，确认新增 AI key 不再在 `zh_CN` 中 pending。

这种策略不削弱激进体验目标。模型仍然可以做感知、记忆、反思和选择，但玩家实际看到的默认文案来自可审计、可翻译、可回归的本地化 lineId。实验模式用于发现好表达，不用于绕过本地化。
...
对当前已实现的 AI 生命层，生产收束模式是默认基线。新增 lineId 进入默认体验前必须至少完成英文源和简中 overlay，不允许先让 `zh_CN` 回退英文。

### 实验模式：dynamicText

模型按玩家当前语言直接生成短句，并在事件中携带 `language`、`modelId`、`policyVersion`、`source`。这是激进原型可以在实验 realm 中试的模式，因为它最能制造“对方真的听懂了我”的感觉。

实验上线条件：

- 增加明确的 AI 动态文本事件类型，例如 `aiSpeech`，不要伪装成普通 `log` 或 `chat`。
- 这是实验 realm 例外，不是默认或正式路径的本地化替代。
- 一旦某类 dynamicText 要进入默认体验，必须回收为 lineId，并补齐 `zh_CN` 等本地化覆盖。
- 增加内容过滤、审计、举报和关停开关。
...
- 超时直接变成 provider error，不阻塞 `server/game.ts` 主循环。
- 连续失败进入 circuit breaker，5 到 10 分钟内暂停新的深推理 job，并向玩家提示 AI 服务暂不可用；不使用本地预写文案冒充 Codex 输出。
- 所有调用写审计：jobId、profileId、trigger、latency、decision status、token usage、provider error 或 validator rejection reason。
...
- 无 Codex CLI、无有效登录、CLI 超时或输出不合法时，能返回明确 provider error、写入审计并展示给玩家。
- 不读取生产仓库 secrets。
- 能演示一个普通怪被点燃成奇点个体。
...
- `AiProvider` 接口先抽象出来，真实实现是 Codex CLI；测试只允许显式注入 provider stub 验证编排，不伪造 Codex CLI。
...
- 单测覆盖 schema parse、超时、损坏 JSON、非法 intent、provider error 和 validator rejection。
...
- 服务端返回新的 `aiSpeech` 或 `aiGossip` 事件，支持 dynamicText experiment 和 lineId only 生产路径。
...
- `tests/ai_orchestrator.test.ts`：队列、缓存、超时、provider error。
...
| i18n 冲突 | 动态文本无法满足当前默认本地化规则。 | 默认路径只收 lineId，实验模式隔离开启，稳定内容先补简中再进入正式体验。 |
...
| 测试不稳定 | AI 输出不可复现。 | 显式 provider stub 只验证编排，fixture replay 和 AI result 持久化为外部输入；真实 Codex CLI 用显式 smoke 验证。 |
...
- 复现方式明确：fixture replay、显式 provider stub、确定性本地规则或固定 seed 至少有一种能稳定复现；不新增或保留运行时替身 Codex provider。
...
- `server/ai/canon_guard.ts` 已把关键任务 NPC、关键任务怪、关键任务物、副本门和奖励来源拆成不同 `CanonGuardSubject`，各自有独立的安全 intent 白名单。
- `dynamicText` 已覆盖英文和中文的任务完成、失败、奖励、经验、金币、声望、任务日志和剧透敏感表达，并额外拒绝链接、HTML 和 Markdown 链接形态。
- `server/ai/intent_validator.ts` 已强制 `line_id_only` 模式拒绝 dynamicText，只有 `dynamic_text_experiment` 或 `mixed_living_world` 才能进入动态文本路径。
- `tests/ai_canon_guard.test.ts` 覆盖关键对象分类、中文动态文本过滤、关键怪、任务物、副本门、奖励来源 intent 限制和 quest hint 可见事实要求。
- `tests/server_ai_interact.test.ts` 已增加 AI 开关双模式主线 parity 测试：同一任务日志、背包、铜币、XP 和等级状态下，AI 开启和关闭分别跑 NPC 问答、物件检视、场景检视和丢弃触发后，主线状态必须完全一致；AI 关闭时不产生 `aiSpeech`，AI 开启时只产生表现事件。
- `tests/ai_intent_validator.test.ts` 覆盖保守模式拒绝 dynamicText，实验模式允许安全 dynamicText 并作为个人 `aiSpeech` 输出。
...
- 可复现：同一 fixture、provider stub 或持久化 AI result 能稳定得到同类结果；失败时必须有明确错误提示和审计记录。
...
- 每个阶段至少做一次人工可玩或脚本化端到端路径，记录入口、操作、观察到的反馈和失败时的错误提示。
...
- 不再新增或保留运行时替身 provider。无 Codex CLI、CLI 超时、schema 错误或输出不合法时，服务端记录 provider error 并向玩家提示明确异常。
...
- 增加 `AI_LIVING_WORLD_EXPERIMENT=0` 显式关闭开关；默认启用安全 lineId 表现层，dynamicText 仍默认关闭。
...
- `tests/ai_orchestrator.test.ts` 覆盖 provider stub 编排、Codex 超时、错误提示和 validator 拒绝路径。
- `tests/server_ai_interact.test.ts` 覆盖交互命令不会崩主循环。
- 手动进入实验 realm，连续点同一 NPC 两次，第二次能引用上一轮摘要或现场事实。

通过标准：

- AI 调用失败时游戏仍可玩，玩家看到明确的 AI 异常，而不是伪造的本地 AI 反馈。
- Codex 正常返回时，玩家至少看到一条来自现场上下文的 NPC 反馈。
- 玩家至少看到一次普通怪产生非模板表现。
...
- `server/ai/codex_worker.ts` 在读取 `codex exec` 输出后会先做本地 `unknown -> AiDecisionV1` 结构收窄，坏 JSON、缺少必填字段、额外字段、非法 `entityRef`、非法 `AiIntentType`、越界 TTL/confidence/dynamicText 都会变成 provider error，不会把未验证对象交给 `intent_validator`。
- `CodexCliProvider` 现在限制失败进程的 stderr 捕获量，避免异常 CLI 输出在服务端内存里无限增长；超时、退出和进程错误会收束成一次 provider error。
- Codex prompt 已明确 `directorProposals` 和 `memorySignals` 只是只读上下文，不是执行 proposal、改世界状态或越过 authority guard 的授权；`tests/ai_codex_worker.test.ts` 直接覆盖本地 parser/schema 收窄，并用 `RUN_REAL_CODEX_CLI=1` 的 smoke 测试真实 `codex exec` 链路。
- `tests/ai_codex_worker.test.ts` 不再伪造 Codex CLI。结构化输出解析覆盖损坏 JSON、结构缺字段、额外字段、非法 intent 名称和 bounded pet intent；真实链路由本机 Codex CLI smoke 验证。
- `AiLifeLayer` 捕获 provider error 后只记录 `provider_error`、写审计和向玩家发送 `AI response failed` 错误事件；validator 拒绝则发送 `AI response rejected`。不再生成本地 profile、物件语义或宠物正则 fallback decision，quest log、已完成任务、战斗、掉落和经济仍不被改动。
...
- 场景快反应和物件本地语义不依赖 Codex，也能稳定复现；物件检视可走 provider/Codex 深推理，但 provider 失败或输出被拒时会回到同一套本地语义外壳。
...
- 同场景没有命中时，NPC 会读取同一大区域内的短 TTL rumor，并以较低强度把它作为跨 NPC、跨小场景的区域传闻提起；跨玩家、跨大区域和过期 rumor 仍被隔离。
- NPC 互动记忆和 rumor 流现在都有明确内存预算：`AiSocialMemoryStore` 会按最近交互裁剪 NPC memory，并按配置上限裁剪 rumor，防止多玩家长时间运行时无限增长；`tests/ai_social_memory.test.ts` 覆盖长流输入只保留最近记录。
- `AiLifeLayer.handleSimEvents` 已接入真实 `questDone` 事件：只有任务已经由 `Sim` 完成后，AI 才会把该事实写成同场景、同来源玩家、短 TTL 的 quest rumor。
- HUD 已支持 `hudChrome.aiSpeech.memoryRecognizesPlayer`、物件 rumor lineId 和任务完成 rumor lineId，任务名通过 `questTitle()` 本地化解析，不把英文任务名从 server 直传到 UI。
- `tests/ai_social_memory.test.ts` 覆盖互动记忆、rumor 衰减、过期、同场景、同区域、跨区域、同来源边界和 quest rumor subjectKind；`tests/server_ai_interact.test.ts` 覆盖真实 `discard` 到 NPC gossip 的同场景和同区域跨 NPC 可玩链路，以及真实 `questDone` 到 NPC 任务传闻的可玩链路，并断言任务状态不被 AI 改动。
- Brother Aldric、The Merchant、Smith Haldren、Scout Maren、Loremaster Caddis、Tidewatcher Ondrel 已接入 profile 级 knowledgeScope、tabooTopics、物件传闻 lineId 和任务传闻 lineId，让记忆与传闻有不同角色口吻。
- 关键 hub 命名 NPC 已补齐第一版生活画像：Marshal Redbrook、Trader Wilkes、Apothecary Lin、Fisherman Brandt、Foreman Odell、Ranger Elwyn、Warden Fenwick、Provisioner Hale、Herbalist Yara、Captain Thessaly、Quartermaster Bree、Armorer Hode 都有独立 persona、fallback lineId、knowledgeScope、tabooTopics、场景偏好、物件兴趣和昼夜天气敏感度。
- 掉落物兴趣评分已读取 NPC profile 的 `itemInterest`，因此草药师会主动关注药水和草药，指挥官会把武器和威胁物当成报告，军需和商人会更容易注意食物、硬币和补给，而不是只靠模板名做浅层判断。
- 场景反应已读取 NPC profile 的 `timeWeatherSensitivity` 和 `sceneAffinities`：观星型角色在非湖边也可能被星空打断，守卫和指挥官在较低亡灵压力下就会警觉，白天、夜晚、雨、雾不再对所有 NPC 使用同一套阈值。
- `tests/ai_social_memory.test.ts` 已增加内容表覆盖门：所有有任务、商店或世界市场入口的交互 NPC 必须解析到非 generic living-world profile，后续新增关键 NPC 时会自动提醒补画像。
- NPC gossip 已加入“询问近况”、“询问传闻”、“询问地点”、“询问任务线索”的实验入口，服务器对 greeting 和主动追问分开限流，主动追问会进入 `topic` 和 `recentObservations`。
- `server/ai/memory_audit.ts` 已把 NPC 互动记忆、物品和任务传闻、世界痕迹、奇点个体记忆、首领遭遇记忆和世界导演状态统一成结构化 `AiMemoryAuditRecord`；`AiDecisionJournal` 会记录本轮实际写入的 `memoryWrites`，`AiJobContextV1.memorySignals` 会把可读记忆信号交给 Codex prompt，不改任务、战斗、掉落、经济或持久角色状态。
- `server/ai/prompt_builder.ts` 已把 `SceneFrameV1` 的附近语义物件显示名、来源、标签和距离，掉落物显示名、标签和新鲜度，随行对象 family 和 tags，近期场景事件，以及昼夜天气 mood 一起写入 Codex prompt。Codex 不再只知道“这里有标签”，而是能看见房屋/锚点、玩家刚丢下的物品、身边宠物和天气情绪，同时仍只能按 `allowedLineIds` 和 `allowedIntents` 输出。
- `server/ai_memory_db.ts` 已提供持久化能力：幂等 `ai_memory_records` DDL、按 realm/player/scene/zone/kind/scope 查询的索引、结构化 audit record upsert、读取归一化、过期清理和当前 realm 的全量 AI memory audit 清理；`ensureSchema()` 已接入该 DDL，测试覆盖 schema、保存读取 round-trip、异常 payload 丢弃、同场景加同区域回读、跨玩家隔离、过期过滤、bounded prune 和 admin 清理用的 realm-scoped delete。
- `AiLifeLayer` 运行时已经把 quest rumor、world trace、world director、NPC interaction、creature memory 和 boss memory 等 `memoryWrites` 接入非阻塞持久化队列；`GameServer.saveAll()` 会等待正在进行的 AI 记忆写入，写入失败只记录 `memoryPersistenceDiagnostics()`，不会阻断当次 AI speech、物件检视、任务完成传闻或首领反馈。
- `AiLifeLayer.pruneExpiredMemory()` 已提供持久化记忆过期清理入口，按 sim time 作为 cutoff，带防重入、批量上限、删除数量指标和失败诊断；`GameServer.start()` 会在 20 Hz tick 外周期性触发清理，`stop()` 清除定时器，不把 SQL 放进主循环。
- NPC 交互、物件检视和场景检视都会按 player、scene、zone、sim time 从 `PgAiMemoryDb` 读取持久化记忆信号；同小场景的 scene 记忆可以精确回读，同大区域的 region 记忆可以在不同小场景回读，跨玩家、跨区域和已过期记录不会进入当前玩家上下文。
- 物件检视会把恢复出的 `worldDirectorState`、rumor、trace 等 audit signal 追加到 `context.memorySignals` 和 `recentObservations`，让 Codex 深推理能看见重启后仍存在的区域传闻或对象印象；场景检视即使没有活跃内存态 director，也能从持久化 `worldDirectorState` 生成只读区域余波表现。
- `tests/ai_memory_persistence.test.ts` 覆盖本阶段可玩链路：丢弃物件后的本地反馈不被 DB 写入拖慢，持久化信号能进入 NPC 和物件 Codex job context，重启式 fresh `AiLifeLayer` 能在场景检视中恢复区域 director echo，持久化故障时玩家仍收到反馈且 pending 写入保留在诊断里，并覆盖过期清理成功、防重入和清理失败诊断；`tests/server_ai_interact.test.ts` 覆盖真实 `GameServer` 定时触发 `PgAiMemoryDb.pruneExpired()`，以及真实 `ai_inspect_scene` 命令通过 `PgAiMemoryDb(pool)` 从持久化 payload 恢复区域 echo，确认使用当前 sim time 且主线状态不变。
- `server/ai/content_coverage.ts` 已提供内容覆盖报告和 `aiContentReviewChecklist()`，覆盖 MobFamily 语义深度、关键 NPC profile 的场景/物件/时间天气/记忆钩子、场景 anchor 和语义物件深度、所有可丢弃物品语义信号、AI lineId 注册情况，并把缺口整理成新增 zone、新族群、新物件和 NPC profile 的人工审查清单；`tests/ai_content_coverage.test.ts` 会在新增内容缺语义时直接失败，也会验证审查清单能指向具体覆盖类别和示例。
- `server/ai/item_interest.ts` 已扩展普通战利品语义：鱼、水边物、皮毛、骨牙、蛛毒、蜡烛、文书、营地杂物、金属财物、图腾仪式物、风暴魔法物和威胁战利品都能进入 dropped item reaction，而不是只剩 generic item kind。

后续仍需维护：

- 已补事务型重启长链路测试：同一持久化链路上 fresh `AiLifeLayer` 能读到重启式区域余波，执行 admin clear 后新的 fresh layer 不再读到旧 director echo，并继续断言 quest log 和已完成任务不变。
- 已补真实 Postgres 长跑验证入口：`tests/ai_memory_pg_longrun.test.ts` 默认跳过，显式设置 `AI_MEMORY_PG_LONGRUN=1` 和 `DATABASE_URL` 后才会连接真实 `ai_memory_records` 表。该测试使用专用短 realm，覆盖 `AiLifeLayer -> PgAiMemoryDb -> ai_memory_records -> fresh AiLifeLayer` 的完整链路，包括 AI 关闭时不写持久记忆、多玩家隔离、多区域保存、多次 flush、过期 prune、重启式 director echo 回读、admin clear 后不回流，并继续断言 quest log 和已完成任务集合不变。PowerShell 本地运行例：`$env:AI_MEMORY_PG_LONGRUN='1'; $env:DATABASE_URL='postgres://eastbrook:change-me@127.0.0.1:5433/eastbrook'; npx vitest run tests/ai_memory_pg_longrun.test.ts`。
- content coverage 已接入结构化审查清单。后续新增实际 zone、新族群、新物件 inspect 或 named NPC profile 时，应同步扩展对应 checklist 条目和人工检查提示。
- 世界导演已补相邻 zone 低强度传播、普通怪跨族群视角投射、同伴投射、NPC 问答本地语义投射、profile 个性化投射、Codex prompt 压缩摘要，以及基于 sim time 的长时段传闻衰减测试。后续仍需随着新增 named NPC 同步扩展 profile projection 分类。
...
- `server/ai/singularity.ts` 已提供确定性的个体 profile，按世界 seed、实体 id、模板和族群生成 `none`、`quirk`、`singularity` 三档，以及食物执念、收藏癖、预兆敏感、胆怯、领地、复仇、观星等首批特质。
- 掉落物和物件检视反应已接入奇点个体偏置，奇点怪会按特质使用专属 lineId，不再只有通用靠近、回避、观察。
- `server/ai/creature_memory.ts` 已提供短 TTL 奇点个体记忆；同一个奇点对象多次目击同一玩家的物品模式后，会追加“认出玩家习惯”的表现层反馈。
- 奇点个体记忆现在会在重复物品模式或重复场景目击后形成短期 `AiCreaturePlan`：例如追随气味、守住地点、避开玩家、观星或读取预兆。计划只写入 server overlay 诊断、world director 证据和 `aiSpeech.reaction` 元数据，并让 HUD 反应徽标获得更明显和更久的计划态表现；它不驱动寻路、仇恨、攻击、掉落、XP、任务或背包。
- `server/ai/family_scene_reactions.ts` 已提供族群场景反应：玩家检视场景时，附近普通怪会按 `FamilySemantics`、场景标签、天气光照、危险度和奇点个体特质产生靠近、回避或观察表现，例如野兽避开 forge 气味，亡灵被 deathPressure 吸引，元素或观星奇点被星空牵动。
- 场景检视现在也会点亮奇点个体记忆：同一个奇点普通怪反复在同一场景语义中注意到玩家时，会触发 `singularityRemembersScene` 表现，携带 sceneId、个体特质和场景标签，并把 `creatureMemory` 写入 journal 和持久化队列；这仍然只影响表达、记忆和审计，不改仇恨、寻路、掉落、XP 或任务。
- 奇点个体的 AI speech 已携带 `individualAlias` 和 `speakerTemplateId`，HUD 会把特质映射成本地化展示称呼，例如饥饿、收藏、预兆、胆怯、领地、复仇和观星倾向对应不同短称呼；这只改变聊天来源和台词里的展示名，不改实体真实名称、目标框、仇恨、掉落、XP 或任务。
- NPC 传闻系统会识别奇点 lineId，把“玩家丢下了某物”的普通传闻升级成“附近某个对象像醒了一下”的特殊回声。
- 奇点生物记忆现在也有全局预算：`AiCreatureMemoryStore` 会按最近目击裁剪旧记录，避免大量普通怪被点亮后撑大服务器内存；`tests/ai_creature_memory.test.ts` 覆盖长流输入只保留最近个体。
- `server/ai/profiles.ts` 已增加通用 `mob.generic.living_world` profile，把普通怪的 provider 输出限制在物品兴趣、族群场景反应和奇点记忆相关 lineId，以及 `lookAt`、`faceEntity`、`pause`、`commentOnScene`、`inspectObject`、`approachObject`、`avoidObject`、`seekShelter` 等表现层 intent 内。
- 奇点普通怪现在会在玩家丢弃物品、检视场景并触发族群场景反应时提交 `singularity_candidate` job 给 provider 或 Codex CLI。provider 可以在本地语义给出的候选 lineId 内“加深”表达；本地 shell 仍保留 `targetItemId`、`sceneTags`、`individualTier`、`individualTraits`、`speakerTemplateId` 和 `individualAlias`，所以 HUD 和审计能看到真实发生的物件、场景和个体特质。
- 重复触发形成的 `AiCreaturePlan` 会作为 `creaturePlan`、`planIntensity` 和 `planEvidence` 写入 `singularity_candidate` 的 recent observations，让 Codex CLI 能围绕当前短期动机做深推理，而不是只看一次性的物品或场景分数。
- 有明确玩家触发源的短期计划还会把 `targetEntityId` 合并回 `aiSpeech.reaction`：追随气味、收藏物件、守住地点、避开玩家或读取预兆时，已有 HUD/renderer 注意力系统会让奇点生物短暂看向触发玩家；观星计划不伪造实体目标。这个目标仍是表现层元数据，只驱动客户端转身、连线和徽标，不写入实体朝向、寻路、仇恨或任务结算。
- 场景物件语义从单层 `tags` 扩展为 `tags`、`featureTags` 和 `affordanceTags`：例如 Smithy House 会携带亮窗、烟囱烟、柴堆，以及避雨、求助、靠门休息等可感知动作倾向；Crypt Gate 会携带锈栏、冷风、骨尘，以及迟疑、守门、逃离黑暗等倾向。这些字段会进入 Codex prompt 的 Nearby semantic objects，并让本地 coverage 检查防止新物件只有粗标签。
- 普通怪族群场景反应会把时间/天气 mood 变成轻量评分偏置和 `reasonTags`：白天活跃度会轻微提高非亡灵/非恶魔/非蜘蛛的好奇，夜晚疲惫会让活物更谨慎但让亡灵和恶魔更被吸引，雨天会让多数族群烦躁、让鱼人和元素更舒适，雾天会增加多数族群的不安；这些都只影响 `aiSpeech` 反应和审计证据，不进入战斗或任务结算。
- 普通怪族群场景反应现在会直接读取附近语义物件的 `featureTags` 和 `affordanceTags`：野兽会被食物气味和鱼鳞吸引、避开冷坟门和热炉火，鱼人会靠近湿木、鱼味和水线，蜘蛛会利用芦苇和低可见度，类人会评估避雨、守门、读告示和求助点。真实地面物件会作为 `targetObjectId` 进入 `aiSpeech.reaction`，纯场景锚点只作为 `targetItemId` 和 `reasonTags` 进入审计与表现，不写入寻路、仇恨、任务、拾取、掉落、XP 或经济。
- 普通怪族群场景反应现在还会读取活跃 world director proposal：source trace 过期后，区域里仍残留的 `echoTrace`、`raiseCampCaution`、`echoEncounterMemory` 等低风险提案会变成靠近、回避或观察的评分偏置和 `director:*` reasonTags。表现仍只落在个人 `aiSpeech.reaction`，不会生成新物件、驱动寻路、改仇恨、掉落、XP、任务、背包或经济。
- `server/ai/director_family_projection.ts` 已把 world director proposal 投射成族群视角：同一个 haunted 或 dread 余波会让亡灵产生死亡共鸣、让恶魔读成压力和愉悦、让活物回避、让元素和龙裔产生扰动或审判感；同一个 covetous 余波会吸引人形、狗头人、巨魔、食人魔等社会或贪婪族群，但不会让普通野兽突然理解金钱传闻。该投射只改变 `curiosity`、`fear`、`reaction` 和 `reasonTags`，仍不写入战斗、任务、寻路、拾取、掉落、XP、背包或经济。
- 同伴场景反应和 Codex prompt 已复用同一投射：宠物或随行 NPC 会把 `directorProjection:*` 和 `family:*` 写入表现层 `sceneTags`，Codex prompt 也会为当前 NPC 或怪物输出 `Director family projection` 压缩摘要，让模型不只看到原始 proposal，还能看到“这个族群会怎么读它”。这些字段仍是只读语义，不是执行 proposal 或改变世界状态的授权。
- NPC 问答的本地语义投射已接入同一套上下文：玩家询问地点、传闻或任务提示时，Codex prompt 会带上投射后的族群/场景摘要，非 provider 的局部场景反应则用 `source: local` 表达，而不改任务、奖励、剧情解锁或世界状态。
- `server/ai/profile_projection.ts` 已从 named NPC profile 的 persona、knowledgeScope、tabooTopics 和 socialMemory 风格推导 `profileProjection:*` 标签：牧师会把余波读成仪式或预兆，商人读成交易天气，守卫读成巡逻风险，草药师读成症状，渔夫或潮汐观察者读成水声线索，铁匠读成钢铁判断，学者读成档案理论。这些标签进入 NPC 问答上下文和 Codex prompt，只影响审计、表现和模型上下文。
- `AiDecisionV1.intents` 现在支持受限目标字段 `targetEntityId`、`targetObjectId`、`targetItemId` 和 `seconds`。Codex CLI 可以让 `lookAt`、`faceEntity`、`inspectObject`、`approachObject` 或 `avoidObject` 指向 job.json 中可见的玩家、当前实体、同伴或附近实体物件；validator 会拒绝不可见目标，并只把合法目标转成 `aiSpeech.reaction` 元数据，让 HUD/renderer 做短暂凝视或注意力连线。
- provider 失败时，奇点反应记录 `provider_error` 并向玩家发送 `AI response failed`；输出被验证器拒绝时记录 `rejected` 并发送 `AI response rejected`。不再用本地语义 lineId 替代 Codex 输出；短期记忆和世界导演状态仍可写入审计，但不改仇恨、寻路、掉落、XP、任务、背包或经济。
- `tests/ai_creature_memory.test.ts` 覆盖奇点场景记忆事件、短期计划生成和计划上限；`tests/ai_world_director.test.ts` 覆盖 world director 证据读取计划类型；`tests/server_ai_interact.test.ts` 覆盖真实 `ai_inspect_scene` 两次触发场景记忆和短期计划，并断言 quest log 和已完成任务集合不变。
- `tests/server_ai_interact.test.ts` 已覆盖奇点物品反应 provider 成功、provider 失败错误提示、奇点场景反应 provider 成功，并断言主线任务、等级和 XP 不被 AI 额外改变；`tests/ai_codex_worker.test.ts` 通过 parser/schema 单测和显式真实 Codex CLI smoke 覆盖 worker 链路。
- 当前基线仍只写 server overlay 事件和短 TTL rumor，不改怪物目标选择、leash、仇恨、伤害、掉落、XP 或任务计数。
...
- 玩家可通过独立 `Inspect Nearby` 键位检视附近地面物件和副本门，`F` 键拾取、NPC 交互和进出副本逻辑保持原样。
- 当附近没有可检视物件时，`Inspect Nearby` 会回退为检视当前小场景，按 forge、ruined chapel、lake dock、watchpost、crypt entrance 和 generic 场景输出本地化反馈。
- `SceneFrameV1.nearbySemanticObjects` 已包含静态建筑和环境锚点，例如炉膛、铁匠屋、集市摊、破钟、墓穴门、码头木板、湖边小屋、塔墙和信号火。
- `server/ai/object_reactions.ts` 已按 forge、grave、lake、dungeon door、singularity、generic 场景语义沉淀首批 lineId 反馈。
- `server/ai/scene_inspection.ts` 已把场景 anchor 和环境标签沉淀为可测试的 scene inspect lineId，当前只发个人 `aiSpeech`，不生成物件、不改任务、不进副本。
- `AiLifeLayer.handleObjectInspection` 已把 `object_inspected` 接入 provider/Codex、intent validator、scene frame、decision journal、短期 item rumor 和附近对象兴趣反应；provider 成功时 `aiSpeech.source` 为 `codex`，同时会合并本地物件 `targetObjectId`、`targetItemId` 和场景 reaction 元数据，保证 HUD/renderer 仍能表现注意目标。
- 物件 provider 失败或输出被拒时，会记录 provider error 或 rejected decision，并向玩家发送明确错误事件；不再回退成本地物件语义 lineId，也不会改变拾取、任务、副本门、掉落、XP、经济或角色持久状态。
- 检视过的物件会在同场景、同来源玩家范围内形成短 TTL rumor，后续 NPC gossip 可以用 profile 口吻提起它。
- 附近 NPC 或怪物会基于同一个物件语义产生靠近、回避或观察反应，但不改变物件拾取、任务状态、掉落、XP、经济和副本权限。
- `tests/ai_object_reactions.test.ts` 覆盖 forge、lake、singularity 的语义分流；`tests/server_ai_interact.test.ts` 覆盖检视墓地物件不拾取、不改任务、provider 成功时 source 为 `codex` 且保留 reaction metadata、provider 失败时提示错误、检视副本门不进本，以及检视物件后 NPC 产生即时反应并在 gossip 中提起。
- `server/ai/world_traces.ts` 已提供服务器内存态短期 world trace store，按同场景、同来源玩家、TTL 和 trace 上限保存，不写入 sim 实体、不持久化角色 JSONB。
- 玩家丢弃食物、诅咒物、贵重物或触发奇点反应后，`AiLifeLayer.handleItemDiscarded` 会把真实发生过的物品语义写成短期 scene trace。
- 普通奇点怪死亡或击杀玩家后也会留下短期 `singularity` scene trace：`AiLifeLayer.handleSimEvents` 在收到普通怪 `death` 事件时按确定性个体 profile 判断是否为奇点，若是则写入 `worldTrace`、`worldDirectorState` 和 journal。玩家随后检视同一场景会看到 `sceneTraceSingularity`，HUD 使用该生物显示名而不是内部 id；这不生成额外掉落、不改 XP、任务、仇恨、leash 或角色持久状态。
- `AiLifeLayer.handleSceneInspection` 会优先读取未过期 trace，让玩家通过检视环境读到刚刚留下的痕迹；trace 过期或跨场景、跨玩家时自动回到静态场景 anchor。
- 检视未过期 trace 时，附近 NPC 或怪物会基于原物品语义再次产生靠近、回避或观察反应，让环境痕迹不只是玩家独白。
- NPC gossip 会把同场景未过期 world trace 写入 `recentObservations`，并追加 trace-specific lineId 反馈，让 NPC 能主动谈到刚刚改变过的环境。
- 首批 trace lineId 覆盖 singularity、cursed、food、valuable、generic 五类，HUD 通过 `t()` 和物品名解析显示，不使用动态硬编码文本。
- `tests/ai_world_traces.test.ts` 覆盖 trace 分类、lineId、TTL、同场景和同来源边界；`tests/ai_scene_inspection.test.ts` 覆盖 trace 优先于静态 anchor；`tests/server_ai_interact.test.ts` 覆盖真实 `discard` 到 `ai_inspect_scene` 的可玩链路，以及奇点怪死亡或击杀玩家后再检视场景读到 singularity trace，并断言不新增物件、不改任务状态或主线快照。
...
- `SceneFrameV1.companions` 现在除受控宠物外，也会把近旁关键 NPC 纳入场景帧：有任务的 NPC、高地位 NPC、商贩类 NPC 和受伤 NPC 会带着 `npc`、`humanoid`、`questNpc`、`highStatus`、`vendor`、`injured`、`escortLike` 等语义标签进入同伴列表。
- NPC 自身被交互时会从自己的 scene frame 里排除，避免 NPC 把自己误判为随行同伴；玩家主动检视场景时，近旁关键 NPC 仍可作为随行者对亡灵压力、天气、星空和安全区作出表达。
- `server/ai/companion_reactions.ts` 已提供表现层同伴情绪反应，按 scene frame 中的 `companions`、天气、光照、亡灵压力和危险密度选择 lineId。
- `AiLifeLayer` 已在 NPC gossip、物件检视和场景检视三条 AI 入口追加同伴反应事件，不写入 sim 状态，不改变宠物命令、仇恨、伤害和战斗规则。
- 非亡灵、非恶魔同伴在 `deathPressure`、`undeadMemory` 或高 `undeadPressure` 场景中会表现害怕；雨天、星空和危险夜晚已有首批 lineId。
- NPC 已能对恶魔同伴进入城镇、岗哨或教堂类场景产生警觉，也能对亡灵同伴进入明亮或生活化场景产生不适。
- 恶魔和亡灵同伴也有自身表达：恶魔在秩序化场景会显得挑衅，亡灵在明亮的生活场景会显得空洞。
- 同伴自身反应已扩展到更多 `MobFamily`：野兽会对死亡、血和火有嗅觉不安；鱼人在雨、水边和鱼腥味中变得兴奋；蜘蛛会在雾和低能见度中进入静止观察；元素会回应天气、星空、火光和高地空气；龙裔会被高处、古老石构和星空牵动；人形、狗头人、巨魔、食人魔等类人随行者会在白天安全区表现出短暂放松。
- `companionReactionEvents` 现在返回 lineId 与 `inspect` 或 `avoid` 表现类型，避免把鱼人遇水、元素共鸣、龙裔观星等正向好奇误标成回避。
- 同伴反应现在也会读取活跃 world director proposal：具体 source trace 过期后，宠物或关键 NPC 仍能对区域 mood 做出嗅探、害怕、共鸣或观察反馈，并把 `director:*` 证据写入 `aiSpeech.reaction.sceneTags`。这不改变宠物模式、命令、仇恨、伤害、任务、背包或经济。
- 宠物自然语言命令已有第一版受限路由：`ai_command_pet` 和 `/petai ...` 会走 AI provider/Codex job，把玩家语言映射到 `commandPetPassive`、`commandPetDefensive`、`commandPetAggressive`、`commandPetAttack`、`commandPetTaunt` 或 `commandPetIgnore`，最终只调用现有 `pet_mode`、`pet_attack`、`pet_taunt` 权威命令。
- `validateAiDecision` 已强制检查当前 job 的 `allowedIntents`，所以 generic mob profile 可以知道宠物命令，但普通怪场景、物件或奇点 job 未显式授权时不能输出宠物控制 intent。
- `tests/ai_scene_semantics.test.ts` 覆盖关键 NPC 和受伤 NPC 进入 scene frame，以及 NPC 自身交互时的排除规则；`tests/ai_companion_reactions.test.ts` 覆盖亡灵压力、亡灵同伴豁免、雨天、星空、恶魔和亡灵同伴自我反应，以及野兽、鱼人、蜘蛛、元素、龙裔和类人随行者的家族化反应；`tests/ai_scene_reactions.test.ts` 覆盖 NPC 对恶魔和亡灵同伴的场景警觉；`tests/ai_pet_command.test.ts` 覆盖宠物命令文本规范化；`tests/server_ai_interact.test.ts` 覆盖带野兽宠物检视墓地物件、关键 NPC 随玩家检视亡灵压力场景、直接检视亡灵压力场景，以及 provider 成功和失败错误提示下的真实宠物命令链路。
...
- `server/ai/boss_memory.ts` 已提供服务器内存态短期遭遇记忆，覆盖 boss、elite、rare 三类强交互对象，按场景、来源玩家、模板和结果隔离，带 TTL、热度和证据列表。
- `AiLifeLayer.handleSimEvents` 已接入真实 `SimEvent` 流：玩家或宠物击败首领/精英/稀有怪会写入 `defeated` 记忆；首领/精英/稀有怪击倒玩家会写入个人 `wipe` 记忆。
- 同一 `SimEvent` 流已支持真实伤害阶段 cue：首领、精英或稀有怪被打到 50% 以下会触发 `bloodied` 表现，被打到 20% 以下会触发 `desperate` 表现，并按实体、来源玩家和阶段去重。
- 遭遇记忆会生成个人 `aiSpeech`，并同步喂给 `world_director`，让后续场景检视或询问 NPC 地点时能读到“首领被击败”或“首领刚压过玩家”的区域余波。
- 当前基线仍只写 server overlay、个人表现事件和短 TTL director state，不改命中、伤害、掉落、XP、任务计数、仇恨、leash、副本权限和持久角色 JSONB。
- HUD 已支持 boss memory、boss phase 和 world director boss echo 的 lineId，并通过 `bossTemplateId` 走本地化首领名解析。
- `tests/ai_boss_memory.test.ts` 覆盖遭遇分类、来源隔离、过期、阶段 cue 去重和个人表现事件；`tests/ai_world_director.test.ts` 覆盖遭遇记忆进入 director state；`tests/server_ai_interact.test.ts` 覆盖真实 `dealDamage -> damage/death -> tick events -> AI` 的阶段、击败和玩家死亡链路，并断言任务状态不变。
...
- `tests/ai_orchestrator.test.ts` 覆盖 director queue、预算、过期、provider error。
...
- `server/ai/world_director.ts` 已提供服务器内存态短期 director state，读取 world trace 和奇点个体短记忆，生成带来源、热度、proposalType、证据列表和 TTL 的区域 mood。
- 首批 mood 覆盖 uncanny、haunted、hungry、covetous、stirred，对应 campAlert、npcTopicShift 和 traceEcho 等低风险 proposal 类型。
- 每个 director state 现在会附带结构化 `proposal` 预览：包含建议意图、目标引用、强度、建议 lineId、证据标签、过期时间和安全说明。当前 proposal 只进入诊断和审计，不自动执行迁移、刷新、任务、战斗、掉落或经济修改，为后续实验 realm 自动执行或 GM 审核队列保留明确边界。
- `AiWorldDirectorStore` 现在保留只读 proposal 生命周期 journal，记录 `created`、`refreshed`、`expired`、`evicted` 四类事件，包含 proposalId、intent、risk、target、scene、zone、lineId、reasonTags 和 safetyNotes；该 journal 进入 `AiLifeLayer` diagnostics 和 admin overview API，但仍不执行 proposal。
- `AiJobContextV1.directorProposals` 已把活跃 proposal 的结构化副本交给 Codex prompt：NPC 询问、奇点物品反应和奇点场景反应都能读取建议意图、风险、目标、证据标签和 safetyNotes，但 validator 不新增任何执行 intent，provider 也只能影响当次 `aiSpeech` 表达。
- 物件检视 job 也会读取同场景或同区域的活跃 director state，把 `worldDirector:*` observation、结构化 proposal 和只读 `worldDirectorState` memory signal 一起交给 Codex；因此墓碑、门、锻炉或地面物件的解释可以感知最近区域余波，但仍只通过当前物件的 allowed lineId 和表现层 intent 输出。
- 物件检视和场景检视现在还会读取持久化 `worldDirectorState` audit：服务端重启或 fresh `AiLifeLayer` 后，同区域的短期 mood 仍能以只读 memory signal、恢复出的低风险 proposal preview 或区域 echo 的方式回到玩家眼前，同时继续按 player、scene、zone、TTL 隔离。
- 场景检视会把活跃 proposal 或从持久化 audit 恢复出的 proposal 传给同伴反应和普通怪族群反应排序，让世界导演的区域 mood 不只出现在玩家/NPC 话题里，也能在重启后继续影响周边生物的即时肢体语言；该路径不执行 proposal，不改变 sim 状态，只生成带目标和 `director:*` 证据的个人 `aiSpeech`。
- 玩家主动询问 NPC 地点时，NPC 可以读取同场景、同来源玩家的 director state 并追加区域状态反馈。
- NPC greeting 现在也能被 `nudgeNpcRumor` 或 `raiseCampCaution` 低风险 proposal 轻推：玩家只是正常打开 gossip，NPC 也可能主动提到当前区域余波，让世界导演不只藏在“询问地点/近况/传闻”的按钮后面。
- 由物品痕迹驱动的 world director state 已携带 `zoneId`，同一大区域内不同小场景的 NPC 地点询问也能读到短期区域余波，例如 forge 留下贵重武器痕迹后，Mirror Lake Dock 的 NPC 也能感到 Eastbrook Vale 的贪婪传闻正在扩散。
- 世界导演现在支持显式开启的相邻 zone 弱传播：Eastbrook Vale、Mirefen Marsh、Thornpeak Heights 按相邻链路读取低强度 echo，proposal 会带 `adjacentZone:*` reasonTag 且强度被压低；默认 store 查询仍不跨区，life layer 的 NPC、物件和场景检视才会读取该弱传播表现。该传播仍按来源玩家隔离，不新增状态、不刷新原状态、不改任务、战斗、掉落、经济或持久角色数据。
- 当源 world trace 已过期但 director state 仍未过期时，玩家检视场景仍能读到短暂区域余温。
- 真实 `questDone` 事件会写入 `relieved` 区域导演状态，玩家在同一大区域的其他小场景询问地点或检视环境时，可以读到任务完成后的区域余波；该状态仍带来源玩家、TTL 和 zone 边界。
- 场景奇点记忆现在也会写入世界导演：同一个奇点普通怪反复在某个场景语义中认出玩家后，会生成 `scene` subject 的 `worldDirectorSceneUncanny` 状态，后续场景检视能读到“这个地方也开始认识你”的余波；该状态不携带 item target，不会被 HUD 当作未知物品显示。
- NPC 询问“近况”现在会补读同场景或同区域的世界导演余波；询问“传闻”时如果具体 rumor 已过期但 director state 还在，NPC 会说区域状态而不是空泛地说没有传闻，让事件从地点按钮渗透到社交话题入口。
- `tests/ai_world_director.test.ts` 覆盖 scene subject 的创建、lineId、事件输出和无 item targeting；`tests/server_ai_interact.test.ts` 覆盖真实 `ai_inspect_scene` 第二次写入 director、第三次读出场景余波，并继续断言 quest log 和已完成任务集合不变。
- `tests/ai_world_director.test.ts` 已增加长流输入预算测试，模拟多区域、多物品事件连续写入时只保留最近 director state，防止 1 到 2 小时事件流让区域状态无限膨胀。
- 当前基线只发个人 `aiSpeech` 和审计友好的 local reaction，不改巡逻、迁移、怪物生成、仇恨、掉落、任务、经济或持久世界状态。
...
- 增加成本仪表：每小时 calls、超时率、provider error 率、平均延迟、spotlight 数量。
...
- 对实验 realm 做多玩家烟测，确认队列、成本、provider error 和审计稳定。
...
- `server/ai/content_coverage.ts` 已提供结构化覆盖报告，统计 MobFamily 语义、交互 NPC profile、场景 anchor、语义物件、关键物品语义和 AI lineId 引用。
- `tests/ai_content_coverage.test.ts` 已把覆盖报告变成守门测试：所有当前 `MobFamily` 必须有语义和内容模板，所有交互 NPC 必须有非 generic living-world profile，主要场景必须有语义对象和标签，关键食物、饮品、药水、武器、任务物必须能解析出语义信号，报告中的 `hudChrome.aiSpeech.*` 必须存在于 HUD chrome catalog。
- `AiLifeLayer.runtimeMetrics()` 已提供第一版运行指标快照，统计 provider calls、成功、错误、旧版 fallback 字段（兼容保留但当前不再递增）、accepted/rejected decision、local reaction、生成事件数、记忆写入队列、持久化写入失败、过期清理次数、清理删除数、清理失败、provider 延迟和最近错误；`tests/ai_decision_journal.test.ts`、`tests/server_ai_interact.test.ts`、`tests/ai_memory_persistence.test.ts` 覆盖成功、拒绝、provider error、本地反应、DB 故障和清理指标。
- `server/ai/intent_validator.ts` 现在把 `aiSpeech.source` 作为调用方显式传入的出处：Codex 成功返回的语音标记为 `codex`，不调用 provider 的确定性场景/记忆反应标记为 `local`。provider error 不再生成 `aiSpeech`，而是发送 `AI response failed` 错误事件；`tests/ai_intent_validator.test.ts`、`tests/ai_codex_worker.test.ts`、`tests/server_ai_interact.test.ts` 覆盖这条语义。
- `/admin/api/overview` 已在 admin bearer token 保护下返回 `ai` 运行指标快照，监控端可以同时读取服务器在线状态、provider usage 和 AI 生命层健康度；`tests/admin.test.ts` 覆盖 overview 返回字段和无 token 拒绝路径。
- admin Usage 页已显示 AI 生命层健康面板，包含 provider errors、旧版 fallback 字段、accepted/rejected decision、local reaction、生成事件、记忆写入、持久化失败、过期清理次数、清理删除数、清理失败和延迟；`tests/admin_ai_metrics_ui.test.ts` 覆盖健康态、异常态和错误文本转义。
- `/admin/api/overview` 现在还返回 `aiCoverage` 和 `aiDiagnostics`：前者是内容覆盖报告，后者是最近 decision journal、世界导演状态和记忆持久化状态的只读快照；两者都复用现有 admin bearer token 保护，不增加玩家写路径。
- admin Usage 页已显示 AI content coverage 面板，能看到族群语义、交互 NPC profile、场景 anchor、语义物件、物品语义和 lineId 引用的覆盖数与缺口示例；缺口 ID 全部转义，`tests/admin_ai_metrics_ui.test.ts` 覆盖覆盖缺口和 XSS 防护。
- admin Usage 页已显示 AI decision diagnostics 面板，能查看最近 decision 的状态、trigger、实体、scene、lineId、intent、memory writes、reason，以及当前 world director mood/proposal/evidence、记忆 flush、记忆 prune 和最近清理数量；`tests/admin.test.ts`、`tests/admin_ai_metrics_ui.test.ts`、`tests/ai_decision_journal.test.ts`、`tests/ai_world_director.test.ts` 覆盖 API、渲染、转义和原有 journal/director 行为。
- admin Usage 页现在会把 world director proposal lifecycle journal 展开成 GM 可读表，显示 created、refreshed、expired、evicted、mood、proposal、intent、目标、场景、强度、reasonTags 和 safetyNotes，让“区域为什么变了、后来为什么消失”不只停留在 API JSON。
- admin Usage 页现在还会把 AI social memory 作为诊断表显示：短期 NPC 互动记忆和 rumor 会按玩家、场景、主题、强度、lineId 展示并转义，GM 能看见“为什么某个 NPC 或区域还在提这件事”。
- `/admin/api/ai/memory/clear` 和 Usage 面板的 `Clear AI memory` 按钮可清理当前进程内的 volatile AI overlay，包括 NPC memory、rumor、world trace、creature memory、creature plan、boss memory、boss phase cue、world director state、decision journal 和 pending memory writes；它现在也会删除当前 realm 的 `ai_memory_records` 持久化 AI memory audit，避免重启后短期传闻、世界痕迹或 director echo 回流。该操作不删除角色存档、不改任务、背包、XP、等级、经济、战斗和宠物状态；持久化删除失败时后台接口返回失败，并在 memory diagnostics 里留下错误，避免运营误判为已经完成重启安全清理。
- `server/ai/content_coverage.ts` 现在还提供 `aiProfilePreviewReport()`，把 authored profile 汇总成有界作者预览：应用对象、persona 摘要、canon sensitivity、fallback lineId、knowledge/taboo 数量、scene/item/time/weather/companion 深度和缺失字段都会被结构化输出。
- `/admin/api/overview` 已返回 `aiProfiles`，admin Usage 页会显示 AI profile preview 表格，让策划和运营能直接看见每个 living-world profile 的作者资料质量，而不是只看覆盖率数字；所有 profile id、templateId、persona、lineId 和缺口字段都会转义，`tests/admin.test.ts`、`tests/admin_ai_metrics_ui.test.ts`、`tests/ai_content_coverage.test.ts` 覆盖 API、渲染、XSS 防护和报告内容。
- `aiProfilePreviewReport()` 现在内嵌 authoring validation，校验 duplicate profile、重复 target、未知 NPC 或 mob target、fallback lineId 不在 allowedLineIds、lineId 命名形状、缺失作者字段和交互 NPC generic fallback；admin Usage 页会显示通过、warning 或 error，并在有问题时展开 issue 表。当前所有 authored profile 在 `tests/ai_content_coverage.test.ts` 下保持 0 error、0 warning。
- AI 审计后台已经拆出专门需求和设计归档：`docs/prd/ai-audit-center.zh_CN.md` 记录缺口、频率统计、token 统计和验收标准，`docs/design/ai-audit-center.zh_CN.md` 记录现有后台能力、持久化审计表、Admin API、Usage 页和测试设计。AI 生命层本文继续作为玩法和系统主方案，审计细节以后以 AI Audit Center 文档为准。
- HUD 已将已有 `aiSpeech.reaction` 的 `approach`、`avoid`、`inspect` 映射成短暂头顶反应徽标，让玩家能在世界里直接看见“感兴趣、警觉、观察”等生命意图；`ignore` 不显示徽标，避免无意义噪音。
- 反应徽标是纯呈现层：读取服务器或本地快反应层已经生成的个人 `aiSpeech` 事件，不写回 `src/sim`，不改变任务、战斗、掉落、仇恨、路径、经济和持久化状态；`tests/ai_reaction_badge.test.ts` 覆盖反应种类到本地化 HUD 文案的映射和忽略路径。
- AI 反应事件现在可以携带 `targetEntityId`，HUD 和 renderer 会把“谁在注意谁”画成短暂注意力连线：附近生物看向被检视物件，NPC 对随行恶魔、亡灵或害怕的活体同伴产生反应时，玩家能直接看到注意目标。
- 注意力连线仍是纯表现层，不改变实体朝向、移动、攻击判定、仇恨或路线；物件自身检视不会画自指连线，只有跨实体注意才显示。`tests/ai_scene_reactions.test.ts` 覆盖同伴目标 ID，`tests/ai_reaction_badge.test.ts` 覆盖物件目标到注意目标的映射。
- renderer 已在注意力连线有效期内为静止、非战斗、非施法、未死亡的 NPC 和怪物添加短暂视觉转身，让它们像是真的看向目标；该转身只覆盖客户端 group 旋转，不写入 `Entity.facing`，不影响 locomotion 判定和服务器结算。`tests/ai_attention_facing.test.ts` 覆盖允许和拒绝条件。
- renderer 现在还会把 `approach`、`avoid`、`inspect` 反应转成短时身体语言：靠近目标、后退避让或轻微探身观察；该偏移只作用于客户端 group 位置，有起落脉冲和距离、移动、战斗、施法、死亡守门，不改变服务器位置、交互范围、碰撞、仇恨、任务或掉落。`tests/ai_attention_facing.test.ts` 覆盖三类偏移和禁用条件。
- 主动社交序列现已支持多句动态口语节拍，而不只是动态开场一句。`server/ai/prompt_builder.ts` 会在 `sequence:social` 上下文里显式要求 2 到 3 条 speech，`server/ai/active_triggers.ts` 会把这些句子按参与者顺序重新分发给不同 NPC 或怪物，并保留原有 thinking beat 和 reaction 元数据。
- 社交序列的 Codex prompt 现在能读取结构化 `sequenceParticipants` roster：每个参与者都有 slot、kind、entityId、templateId 和 name。模型不再只靠 `partnerName` 推断对话对象，因此同一段连续聊天里谁先说、谁回应、第三个旁观者何时插一句，已经有了更清楚的上下文锚点。
- 多句动态文本目前仍被严格约束在社交序列上下文内：`server/ai/codex_worker.ts` 和 `server/ai/intent_validator.ts` 允许 2 到 3 条 speech 进入 `sequence:social`，但普通主动事件继续限制为单句，避免全局主动 AI 一下子变成并发刷屏。
- 社交序列现在还能做“动态前半段 + 本地尾拍”的混合播放。如果 provider 只给了 2 句，而现场原计划是 3 个 beat，调度器会保留后续本地 `lineId` thinking/speech 尾拍，不会把第三位参与者整段吞掉。`tests/ai_active_triggers.test.ts` 已覆盖完整多句分发和局部回退尾拍两条路径。
...
- 可验证：必须有 fixture replay、显式 provider stub 或确定性本地规则，避免“今天刚好模型这么说”；真实 Codex CLI 链路另用 smoke 测试证明可运行。
- 可观察：必须有 audit、decision log、provider error log 和至少一条人工可读原因。
