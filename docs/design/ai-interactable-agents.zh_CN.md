...
- 奇点个体记忆现在会在重复物品模式或重复场景目击后形成短期 `AiCreaturePlan`：例如追随气味、守住地点、避开玩家、观星或读取预兆。计划只写入 server overlay 诊断、world director 证据和 `aiSpeech.reaction` 元数据，不驱动寻路、仇恨、攻击、掉落、XP、任务或背包。
