...
- `tests/ai_companion_reactions.test.ts` 覆盖亡灵压力、亡灵同伴豁免、雨天、星空、恶魔和亡灵同伴自我反应；`tests/ai_scene_reactions.test.ts` 覆盖 NPC 对恶魔和亡灵同伴的场景警觉；`tests/server_ai_interact.test.ts` 覆盖带非亡灵宠物检视墓地物件时的真实服务器事件。
