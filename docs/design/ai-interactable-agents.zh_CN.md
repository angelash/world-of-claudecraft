...
- `server/ai_memory_db.ts` 已提供持久化能力：幂等 `ai_memory_records` DDL、按 realm/player/scene/zone/kind/scope 查询的索引、结构化 audit record upsert、读取归一化和过期清理；`ensureSchema()` 已接入该 DDL，测试覆盖 schema、保存读取 round-trip、异常 payload 丢弃和 bounded prune。
...
- NPC 交互会按 player、scene、zone、sim time 从 `PgAiMemoryDb` 读取持久化记忆信号，并追加到 `context.memorySignals`，让 Codex 深推理能看见重启后仍存在的区域传闻或对象印象。
- `tests/ai_memory_persistence.test.ts` 覆盖本阶段可玩链路：丢弃物件后的本地反馈不被 DB 写入拖慢，持久化信号能进入 NPC Codex job context，持久化故障时玩家仍收到反馈且 pending 写入保留在诊断里，并覆盖过期清理成功、防重入和清理失败诊断；`tests/server_ai_interact.test.ts` 覆盖真实 `GameServer` 定时触发 `PgAiMemoryDb.pruneExpired()`，确认使用当前 sim time。
...
- 增加真实 Postgres 或事务 fake 的长链路保存回读、重启恢复和关闭开关回退测试，覆盖多玩家、多区域和跨 session 的长期运行场景。
