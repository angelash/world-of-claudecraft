...
- `server/ai/memory_audit.ts` 已把 NPC 互动记忆、物品和任务传闻、世界痕迹、奇点个体记忆、首领遭遇记忆和世界导演状态统一成结构化 `AiMemoryAuditRecord`；`AiDecisionJournal` 会记录本轮实际写入的 `memoryWrites`，`AiJobContextV1.memorySignals` 会把可读记忆信号交给 Codex prompt，仍只作为内存态审计和深推理输入，不写数据库、不改任务、战斗、掉落、经济或持久角色状态。
- `server/ai_memory_db.ts` 已提供持久化前置能力：幂等 `ai_memory_records` DDL、按 realm/player/scene/zone/kind/scope 查询的索引、结构化 audit record upsert、读取归一化和过期清理；`ensureSchema()` 已接入该 DDL，测试覆盖 schema、保存读取 round-trip、异常 payload 丢弃和 bounded prune。当前还未把运行时内存 store 自动写入数据库。

后续仍需完成：

- 将运行时 relationship、rumor、scene、creature、world memory 接入 `PgAiMemoryDb`，并增加真实 Postgres 或事务 fake 的保存回读、重启恢复和关闭开关回退测试。
