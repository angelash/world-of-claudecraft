...
- Codex prompt 已明确 `directorProposals` 和 `memorySignals` 只是只读上下文，不是执行 proposal、改世界状态或越过 authority guard 的授权；fake Codex 测试会直接检查临时 `job.json` 和 prompt 中都包含这些深层上下文。
- `tests/ai_codex_worker.test.ts` 覆盖 fake Codex 正常输出、损坏 JSON、结构缺字段、额外字段、非法 intent 名称、stderr 截断，以及从 `AiLifeLayer.handleNpcInteraction` 和 `AiLifeLayer.handleObjectInspection` 真实走 `CodexCliProvider` fake CLI 的链路，确保接入不只是“能调用”，还要能拒绝坏输出、产出可见 `aiSpeech` 并维持 server overlay 的安全边界。
- `AiLifeLayer` 捕获 provider error 后会记录 `provider_error`，再生成本地 profile 或物件语义 fallback decision 并重新走 `validateAiDecision`；真实 server 命令路径已覆盖 Codex worker 超时时玩家仍收到安全 `lineId` 反馈，且 quest log、已完成任务、战斗、掉落和经济不被改动。
...
- 已补事务 fake 型重启长链路测试：同一持久化 fake 上 fresh `AiLifeLayer` 能读到重启式区域余波，执行 admin clear 后新的 fresh layer 不再读到旧 director echo，并继续断言 quest log 和已完成任务不变。
...
- 世界导演已补相邻 zone 低强度传播、普通怪跨族群视角投射、同伴投射、NPC 问答 fallback、profile 个性化投射、Codex prompt 压缩摘要，以及基于 sim time 的长时段传闻衰减测试。后续仍需随着新增 named NPC 同步扩展 profile projection 分类。
...
- NPC 问答 fallback 已接入同一投射：玩家询问地点、传闻或任务提示时，fallback lineId 仍保持本地化和 Canon Guard 边界，但 `aiSpeech.reaction` 会带上投射后的 `kind`、`targetItemId` 和 `directorProjection:*` 审计标签，让 NPC 的普通回答也能读到区域余波，而不改任务、奖励、剧情解锁或世界状态。
- `server/ai/profile_projection.ts` 已从 named NPC profile 的 persona、knowledgeScope、tabooTopics 和 socialMemory 风格推导 `profileProjection:*` 标签：牧师会把余波读成仪式或预兆，商人读成交易天气，守卫读成巡逻风险，草药师读成症状，渔夫或潮汐观察者读成水声线索，铁匠读成钢铁判断，学者读成档案理论。这些标签进入 NPC 问答 fallback 和 Codex prompt，只影响审计、表现和模型上下文。
...
- provider 失败或输出被验证器拒绝时，奇点反应会回退到本地语义 lineId，并记录 `provider_error`、`accepted` 或 `rejected`、`local_reaction` journal；这条路径仍只发 `aiSpeech`、写短期记忆和世界导演状态，不改仇恨、寻路、掉落、XP、任务、背包或经济。
...
- `tests/server_ai_interact.test.ts` 已覆盖奇点物品反应 provider 成功、provider 失败 fallback、奇点场景反应 provider 成功，并断言主线任务、等级和 XP 不被 AI 额外改变；`tests/ai_codex_worker.test.ts` 已覆盖假 Codex CLI 端到端生成奇点物品反应，证明这条路径不是只在 fake provider 中可用。
...
- 物件 provider 失败或输出被拒时，会记录 provider error 或 rejected decision，再回退到本地物件语义 lineId，不会让检视静默失败，也不会改变拾取、任务、副本门、掉落、XP、经济或角色持久状态。
...
- `tests/ai_object_reactions.test.ts` 覆盖 forge、lake、singularity 的语义分流；`tests/server_ai_interact.test.ts` 覆盖检视墓地物件不拾取、不改任务、provider 成功时 source 为 `codex` 且保留 reaction metadata、provider 失败时回退本地语义、检视副本门不进本，以及检视物件后 NPC 产生即时反应并在 gossip 中提起。
...
- `AiLifeLayer.runtimeMetrics()` 已提供第一版运行指标快照，统计 provider calls、成功、错误、fallback、accepted/rejected decision、local reaction、生成事件数、记忆写入队列、持久化写入失败、过期清理次数、清理删除数、清理失败、provider 延迟和最近错误；`tests/ai_decision_journal.test.ts`、`tests/server_ai_interact.test.ts`、`tests/ai_memory_persistence.test.ts` 覆盖成功、拒绝、provider fallback、本地反应、DB 故障和清理指标。
- `server/ai/intent_validator.ts` 现在把 `aiSpeech.source` 作为调用方显式传入的出处：provider 或 Codex 成功返回的 `lineId` 语音标记为 `codex`，provider error 触发的本地降级仍标记为 `fallback`，避免把“本地化 lineId 表达形式”和“是否真实经过模型选择”混在一起；`tests/ai_intent_validator.test.ts`、`tests/ai_codex_worker.test.ts`、`tests/server_ai_interact.test.ts` 覆盖这条语义。
...
- admin Usage 页已显示 AI 生命层健康面板，包含 provider fallback、错误、accepted/rejected decision、local reaction、生成事件、记忆写入、持久化失败、过期清理次数、清理删除数、清理失败和延迟；`tests/admin_ai_metrics_ui.test.ts` 覆盖健康态、异常态和错误文本转义。
