...
- HUD 已支持 `hudChrome.aiSpeech.bossMemoryDefeated`、`hudChrome.aiSpeech.bossMemoryWipe`、`hudChrome.aiSpeech.worldDirectorBossDefeated`、`hudChrome.aiSpeech.worldDirectorBossWipe` 四条 lineId，并通过 `bossTemplateId` 走本地化首领名解析。
- `tests/ai_boss_memory.test.ts` 覆盖遭遇分类、来源隔离、过期和个人表现事件；`tests/ai_world_director.test.ts` 覆盖遭遇记忆进入 director state；`tests/server_ai_interact.test.ts` 覆盖真实 `dealDamage -> death -> tick events -> AI` 的击败和玩家死亡链路，并断言任务状态不变。
