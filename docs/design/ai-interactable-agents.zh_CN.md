...
- `tests/ai_companion_reactions.test.ts` 覆盖亡灵压力、亡灵同伴豁免、雨天、星空、恶魔和亡灵同伴自我反应，以及野兽、鱼人、蜘蛛、元素、龙裔和类人随行者的家族化反应；`tests/ai_scene_reactions.test.ts` 覆盖 NPC 对恶魔和亡灵同伴的场景警觉；`tests/ai_pet_command.test.ts` 覆盖中英文自然语言到受限宠物 intent 的路由；`tests/server_ai_interact.test.ts` 覆盖带野兽宠物检视墓地物件、直接检视亡灵压力场景，以及 provider 成功和失败回退下的真实宠物命令链路。
