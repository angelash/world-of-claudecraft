...
export class GameServer {
  ...
  start(): void {
    ...
    this.aiMemoryPruneInterval = setInterval(() => {
      void this.pruneExpiredAiMemory();
    }, AI_MEMORY_PRUNE_INTERVAL_MS);
  ...
  aiLifeLayerMetrics(): AiLifeLayerMetricsSnapshot {
    return this.aiLifeLayer.runtimeMetrics();
  }
    ...
    switch (msg.cmd) {
      ...
      case 'ai_interact_npc':
        if (typeof msg.npc === 'number') {
          ...
          this.handleAiInteractNpc(session, msg.npc, locale, parseAiNpcInteractionTopic(msg.topic));
        }
        break;
      case 'ai_inspect_object':
        if (typeof msg.object === 'number') {
          const locale = typeof msg.locale === 'string' ? msg.locale : session.locale;
          session.locale = locale;
          this.handleAiInspectObject(session, msg.object, locale);
        }
        break;
      case 'ai_inspect_scene':
        session.locale = typeof msg.locale === 'string' ? msg.locale : session.locale;
        this.handleAiInspectScene(session, session.locale);
        break;
      case 'ai_command_pet':
        if (typeof msg.text === 'string') {
          const locale = typeof msg.locale === 'string' ? msg.locale : session.locale;
          session.locale = locale;
          this.handleAiCommandPet(session, msg.text, locale);
        }
        break;
      case 'loot':
